...
public class KeyboardView extends View {
    ...
    private void onDrawKeyboard(@NonNull final Canvas canvas) {
        ...
        /*Why the heck this ain't workin' */
        // More time wasted here..
    ...
    private void onDrawKey(@NonNull final Key key, @NonNull final Canvas canvas,
        ...
        if (!key.isSpacer()) {
            if(key.getAltCode() != Constants.CODE_CTRL && key.getAltCode() != Constants.CODE_ALT) {
                ...
                switch (key.getAltCode()){
                    case Constants.CODE_CTRL:
                        if(Special.isCtrl) { dr = getContext().getResources().getDrawable(R.drawable.press); }
                        else { dr = mKeyBackground; }
                        break;
                    case Constants.CODE_ALT:
                        if(Special.isAlt) { dr = getContext().getResources().getDrawable(R.drawable.press); }
                        else { dr = mKeyBackground; }
                        break;
                    default:
                        dr = mKeyBackground;
                ...
                onDrawKeyBackground(key, canvas, dr);
            }
        }
    ...
    public StateListDrawable getSelectorDrawable(int color) {
        ...
        out.addState(new int[]{android.R.attr.state_checkable}, createNormalDrawable(color));
        out.addState(new int[]{android.R.attr.state_checked}, createStrokeDrawable(color));
        return out;
    }
    ...
    public Drawable createStrokeDrawable(int color) {
