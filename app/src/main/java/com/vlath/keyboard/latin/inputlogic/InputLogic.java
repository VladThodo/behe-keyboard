...
package com.vlath.keyboard.latin.inputlogic;
...
import com.vlath.keyboard.event.Event;
import com.vlath.keyboard.event.InputTransaction;
import com.vlath.keyboard.latin.LatinIME;
import com.vlath.keyboard.latin.RichInputConnection;
import com.vlath.keyboard.latin.common.Constants;
import com.vlath.keyboard.latin.common.Special;
import com.vlath.keyboard.latin.common.StringUtils;
import com.vlath.keyboard.latin.settings.SettingsValues;
import com.vlath.keyboard.latin.utils.InputTypeUtils;
import com.vlath.keyboard.latin.utils.RecapitalizeStatus;
...
public final class InputLogic {
    ...
    private void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction) {
        switch (event.mKeyCode) {
            ...
            case Constants.CODE_LEFT_ARROW:

                mConnection.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DPAD_LEFT));

                break;

            case Constants.CODE_RIGHT_ARROW:

                mConnection.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DPAD_RIGHT));

                break;

            case Constants.CODE_UP_ARROW:

                mConnection.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DPAD_UP));

                break;

            case Constants.CODE_DOWN_ARROW:

                mConnection.sendKeyEvent(new KeyEvent(KeyEvent.ACTION_DOWN, KeyEvent.KEYCODE_DPAD_DOWN));

                break;

            case Constants.CODE_CTRL:

                //Special.isCtrl = !Special.isCtrl;

                break;

            case Constants.CODE_ARROW_SWITCH:
                break;
            case Constants.CODE_TO_ALPHA:
                break;
    ...
    private void sendKeyCodePoint(final int codePoint) {
        ...
        if(Special.isCtrl) {
            mConnection.sendKeyEvent(new KeyEvent(100, 100, KeyEvent.ACTION_DOWN, getHardKeyCode(codePoint), 0, KeyEvent.META_CTRL_ON));
        }
        else{
